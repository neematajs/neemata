...
module.exports = (registry, config = {}) => {
  ...
  // Concurrency control to ensure only one read happens at a time
  ...
  function collectMemoryStats() {
    // If a read is already in progress, wait for it
    if (readPromise) {
      return readPromise
    }

    // Start a new read and store the promise
    readPromise = readAndProcessMemoryStats()
      ...
      .catch(() => {
        // noop - silently ignore errors
      })
      .finally(() => {
        // Clear the promise so next collect can proceed
        readPromise = null
      })
