...
class LabelMap {
  ...
  constructor(labelNames = []) {
    // Sort label names to ensure consistent key generation regardless of input order
    this.#labelNamesArray =
      labelNames.length > 0 ? labelNames.slice().sort() : labelNames
    this.#labelNames = new Set(this.#labelNamesArray)
  }
   ...
   */
  validate(labels) {
    // Fast path: skip validation if no labels or metric has no label names
    // This is critical for performance of metrics without labels
    if (labels === undefined || this.#labelNames.size === 0) {
      return true
    }
   ...
   */
  keyFrom(labels) {
    // Fast path: check if labels object is empty using for...in
    ...
    const names = this.#labelNamesArray
    const firstValue = labels[names[0]]
    let res = firstValue !== undefined ? String(firstValue) : ''
    for (let i = 1; i < names.length; i++) {
      const labelName = names[i]
      const value = labels[labelName]
      res += value !== undefined ? `|${value}` : '|'
    }
