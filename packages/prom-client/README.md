...
This package lives in the Neemata workspace. It was imported from
`neematajs/prom-client` at `138b740ff5365c886864fb2a8f3c389e6702b242`, including
worker discovery refresh after reloads. Run `pnpm test:prom-client` from the
workspace root. The package retains its Apache-2.0 license and upstream notices.

Workspace changes include formatting and built-in module import normalization,
plus the missing `WorkerRegistry` constructor declaration in `index.d.ts`.
