...
import type { HttpHandlerCorsOptions } from '../../../src/neemata/http/types.ts'
import {
  createTestParams,
  createTestRequest,
  createTestServer,
...
async function preflight(
  ...
  return await server.handle(
    createTestRequest(origin === null ? {} : { origin }, null, 'OPTIONS'),
  )
}
...
describe('CORS Vary header', () => {
  ...
  it('omits Vary: Origin when cors is not configured', async () => {
    ...
    const response = await server.handle(
      createTestRequest({ origin: ORIGIN }, null, 'OPTIONS'),
    )
  ...
  it('merges Vary from custom rpc responses instead of overwriting', async () => {
    ...
    const response = await server.handle(createTestRequest({ origin: ORIGIN }))
