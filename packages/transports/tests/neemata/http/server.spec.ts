import { BaseServerCodec, ProtocolCodecRegistry } from '@nmtjs/protocol/server'
import { describe, expect, it, vi } from 'vitest'

import type { NeemataHttpRequest } from '../../../src/neemata/http/types.ts'
...
async function createServer() {
  ...
  const server = new NeemataHttpHandler(
    params as any,
    new ProtocolCodecRegistry([codec]),
    { path: '/' },
  )
...
describe('NeemataHttpHandler.handle', () => {
  ...
  describe('GET Accept negotiation', () => {
    ...
    it('falls back to the default codec when Accept is not negotiable', async () => {
      const { server } = await createServer()
