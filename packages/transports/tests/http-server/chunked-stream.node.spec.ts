...
import { writeChunkedBody } from '../../src/http-server/node.ts'

describe('node host', () => {
  describe('writeChunkedBody writable dispatcher', () => {
    ...
    it('resumes exactly one pending waiter per drain across cycles', async () => {
      ...
      const pump = writeChunkedBody(double.res, chunks(4))
    ...
    it('writable callback without a pending waiter is a no-op returning true', async () => {
      ...
      const pump = writeChunkedBody(double.res, chunks(1))
    ...
    it('abort while a waiter is pending settles the wait and exits the pump', async () => {
      const double = createResDouble([false])
      const body = chunks(2)
      const pump = writeChunkedBody(double.res, body)
    ...
    it('handles a drain cycle followed by an abort', async () => {
      ...
      const pump = writeChunkedBody(double.res, chunks(3))
