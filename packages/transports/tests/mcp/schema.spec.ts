...
import { emitInputSchema } from '../../src/mcp/schema.ts'
...
describe('MCP input schemas', () => {
  it('describes the incoming wire shape of codecs and directional schemas', () => {
    ...
    for (const schema of [codec, codec.decode]) {
      expect(emitInputSchema(schema)).toMatchObject({
        kind: 'object',
        schema: {
          properties: {
            at: {
    ...
    expect(emitInputSchema(z.object({ count: z.number() }))).toMatchObject({
      kind: 'object',
  ...
  it('distinguishes missing input from an explicitly impossible input', () => {
    expect(emitInputSchema(undefined)).toEqual({
      kind: 'none',
      schema: { type: 'object', properties: {}, additionalProperties: false },
    })
    expect(() => emitInputSchema(t.never())).toThrow('must be object schemas')
    expect(() => emitInputSchema(t.any())).toThrow('must be object schemas')
  })
