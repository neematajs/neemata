...
import type { ServerHost } from '../../http-server/types.ts'
import type {
  NeemataWebSocketHandlerOptions,
  NeemataWebSocketOptions,
...
export function neemataWebSocket({
  codecs,
}: NeemataWebSocketOptions): ServerHandler<
  NeemataWebSocketHandlerOptions,
  typeof injections,
...
export class NeemataWebSocketHandler {
  ...
  private async upgrade(req: Parameters<NonNullable<Hooks['upgrade']>>[0]) {
    ...
    try {
      ...
      const { encoder, decoder } = negotiateCodecs(this.#codecs, {
        accept,
        contentType,
      })
