...
import { parseRuntimeStartResult } from '../../src/internal/schemas/runtime.ts'
...
describe('Neem internal schemas', () => {
  ...
  it('rejects runtime upstreams with unsupported transport types', () => {
    expect(() =>
      parseRuntimeStartResult([{ type: 'tcp', url: 'http://127.0.0.1:3000' }]),
    ).toThrow(/type/)
  })

  it('rejects runtime upstreams with invalid URLs', () => {
    expect(() =>
      parseRuntimeStartResult([{ type: 'http', url: 'not-a-valid-url' }]),
    ).toThrow(/url/)
  })
