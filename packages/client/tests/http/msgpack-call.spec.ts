...
import { MsgpackCodec as ClientMsgpackCodec } from '@nmtjs/protocol/msgpack/client'
import { describe, expect, it, vi } from 'vitest'

import { HttpTransportFactory } from '../../src/transports/http/index.ts'

describe('HttpTransportClient + MsgpackCodec', () => {
  const codec = new ClientMsgpackCodec()
    ...
    const fetchSpy = vi
      ...
      .mockResolvedValue(
        new Response(
          ...
          { status: 200, headers: { 'content-type': codec.contentType } },
        ),
      )

    return new StaticClient(
      { contract: {} as any, protocol: ProtocolVersion.v1, codec },
  ...
  it('decodes explicit MessagePack null as null', async () => {
    const client = createClient(codec.encode(null))
  ...
  it('omits undefined object properties and preserves null', async () => {
    const client = createClient(codec.encode({ a: 1, b: undefined, c: null }))
  ...
  it('converts undefined array entries to null', async () => {
    const client = createClient(codec.encode(['x', undefined, null, 'y']))
