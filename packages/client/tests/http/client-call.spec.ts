...
} from '../../src/transports/http/index.ts'

class TestJsonCodec extends BaseClientCodec {
...
describe('HttpTransportClient + StaticClient', () => {
  ...
  it('returns decoded object for unidirectional RPC call', async () => {
    ...
    const fetchSpy = vi
      ...
      .mockImplementation(async (_url, init) => {
        ...
        const payload = codec.decode(requestBody)
        const responseBody = codec.encode({ ok: true, echoed: payload })
  ...
  it('omits keepalive by default', async () => {
    const codec = new TestJsonCodec()
    const fetchSpy = vi.fn<typeof fetch>().mockResolvedValue(
      new Response(codec.encode({ ok: true }) as any, {
        status: 200,
        headers: { 'content-type': codec.contentType },
      }),
    )
  ...
  it('maps non-ok responses into client protocol errors', async () => {
    ...
    const fetchSpy = vi.fn<typeof fetch>().mockResolvedValue(
      new Response(
        codec.encode({ code: 'BAD_REQUEST', message: 'Nope' }) as any,
        {
          status: 400,
