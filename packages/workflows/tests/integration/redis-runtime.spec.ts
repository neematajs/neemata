...
for (const target of targets) {
  ...
  describe.skipIf(!target.url)(
    ...
    () => {
      ...
      it('runs a workflow through pools, env and a codec pair', async () => {
        const { runtime } = createHarness()
        const date = {
          decode: z.iso.datetime().transform((stored) => new Date(stored)),
          encode: z.date().transform((value) => value.toISOString()),
        }
        type Clock = { readonly shiftMs: number }
        const shiftTask = defineTask({
          name: `shift-${randomUUID()}`,
          input: date,
          output: date,
        })
        const shift = implementTask(shiftTask, {
          pool: 'heavy',
          handler: (input, _lifecycle, env: Clock) =>
            new Date(input.getTime() + env.shiftMs),
        })
        const workflow = defineWorkflow({
          name: `shifted-${randomUUID()}`,
          input: date,
          output: date,
        })
          .task('shifted', shiftTask)
          .activity('again', { input: date, output: date })
          .build()
        const implementation = implementWorkflow(workflow, { pool: 'light' })
          .shifted(shiftTask, { input: (_outputs, input) => input })
          .again(
            (input, _lifecycle, env: Clock) =>
              new Date(input.getTime() + env.shiftMs),
            { input: ({ shifted }) => shifted },
          )
          .finish(({ again }) => again)
        const workers = {
          ...runtime,
        ...
        // The task belongs to another pool, so this pool alone cannot finish.
        ...
        expect(snapshot?.run.output).toBe('2026-01-01T00:00:02.000Z')
      })
