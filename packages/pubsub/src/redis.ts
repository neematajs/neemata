...
export class RedisPubSubAdapter implements PubSubAdapter {
  ...
  protected subscriberClient?: RedisPubSubClient

  constructor(
    protected readonly client: RedisPubSubClient,
    logger?: Logger,
  ) {
    this.logger = logger?.child({ component: RedisPubSubAdapter.name })
  }
  ...
  async initialize() {
    ...
    this.subscriberClient = this.client.duplicate() as RedisPubSubClient
