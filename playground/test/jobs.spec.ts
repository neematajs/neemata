...
const backend = process.env.JOBS_BACKEND === 'valkey' ? 'valkey' : 'redis'
const CONFIG_PATH = resolve(CWD, `neemata.jobs.${backend}.config.js`)
...
describe('Playground E2E - Jobs', { timeout: 60000 }, () => {
  let serverProcess: ChildProcess | null = null

  beforeAll(async () => {
    serverProcess = await startServer('preview', { configPath: CONFIG_PATH })
  }, 30000)
  ...
  it('slow job can be canceled and reaches failed status', async () => {
    ...
    try {
      const queued = await client.call.startSlowJob({ ticks: 80, delayMs: 60 })
      ...
      const failed = await poll(async () => getJob(client, 'slow', queued.id), {
  ...
  it('checkpoint job fails first, then retry clearState:false resumes and completes', async () => {
    ...
    try {
      const total = 6
      const queued = await client.call.startCheckpointJob({ total, failAt: 2 })
  ...
  it('slow job persists progress (>0) before completion or cancel', async () => {
    ...
    try {
      const queued = await client.call.startSlowJob({ ticks: 40, delayMs: 80 })
